    ...
    <section ref={ref} className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      >
        <motion.div 
          ...
          className="mb-4"
        >
          {/* Made Panja Solutions bigger: changed from text-sm to text-lg and added md:text-xl */}
          <span className="text-[#95C1D9] font-medium tracking-[0.2em] uppercase text-3xl md:text-4xl drop-shadow-lg">
            Panja Solutions
          </span>
        </motion.div>

        {/* Made Engineering Digital Excellence smaller: changed from text-5xl to text-4xl, md:text-7xl to md:text-6xl, lg:text-9xl to lg:text-8xl */}
        <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold text-white tracking-tight mb-8 drop-shadow-2xl">
          <motion.span 
            className="block"
        ...
        <motion.p 
          className="max-w-xl mx-auto text-lg md:text-xl text-white/90 leading-relaxed drop-shadow-lg"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2">
        {heroImages.map((_, index) => (
          <button 
            ...
            className={`w-2 h-2 rounded-full transition-all duration-500 ${index === currentSlide ? 'bg-white w-8' : 'bg-white/30 hover:bg-white/50'}`}
